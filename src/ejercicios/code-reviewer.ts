...
import { streamGemini } from "../llm/gemini-client.js";
...
async function reviewFile(filePath: string): Promise<void> {
    ...
    if (!SUPPORTED_FILE_EXTENSIONS.has(fileExtension)) {
        console.warn(`Esta extension " ${fileExtension} no reconocida. Continuando de todas formas...`);
    }
    ...
    **Líneas:** ${totalLines}
    ...
    await streamGemini(prompt, CODE_REVIEWER_PROMPT);
    console.log("─".repeat(60));
