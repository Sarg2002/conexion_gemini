...
function validateProvider(provider: string): "anthropic" | "openai" | "gemini" {
    if (provider === "anthropic" || provider === "openai" || provider === "gemini") {
        return provider;
    }
    throw new Error(`MODEL_PROVIDER invalido: ${provider}. Debe ser "anthropic", "openai" o "gemini"`);
}
...
export const config: AppConfig = {
    ...
    geminiModel: getRequiredEnvVar("GEMINI_MODEL", "gemini-3.6-flash"),
...
export function validateConfig(): void {
    if (config.provider === "anthropic" && !config.anthropicApiKey) {
        throw new Error("ANTHROPIC_API_KEY está vacía. Agregala en tu archivo .env");
    }
    if (config.provider === "openai" && !config.openaiApiKey) {
        throw new Error("OPENAI_API_KEY está vacía. Agregala en tu archivo .env")
    }
    if (config.provider === "gemini" && !config.geminiApiKey) {
        throw new Error("GEMINI_API_KEY está vacía. Agregala en tu archivo .env")
    }
}
